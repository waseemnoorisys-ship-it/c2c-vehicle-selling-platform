...
import { verifyPkce, createAccessToken, } from "./oauth.token.service.js";
import { createOAuthSession, getOAuthSession, authenticateOAuthSession, consumeAuthorizationCode, registerOAuthClient, getOAuthClient, } from "./oauth.service.js";
...
router.get("/authorize", async (req, res) => {
    try {
        const { response_type, client_id, redirect_uri, scope, state, code_challenge, code_challenge_method, redirect_to_frontend, } = req.query;
...
router.get("/login", async (req, res) => {
    ...
    const frontendUrl = process.env.FRONTEND_URL || "https://c2c-vehicle-selling-platform.vercel.app";
    const publicBaseUrl = process.env.MCP_PUBLIC_URL ||
        (process.env.RENDER_EXTERNAL_HOSTNAME
            ? `https://${process.env.RENDER_EXTERNAL_HOSTNAME}`
            : "http://localhost:3001");
    const webLoginUrl = `${frontendUrl}/login?mcp_session=${encodeURIComponent(session.sessionId)}&mcp_callback=${encodeURIComponent(`${publicBaseUrl}/oauth/authorize/callback`)}`;
    return res.send(`
    <!DOCTYPE html>
    <html lang="en">
      <head>
...
router.post("/authorize/direct-login", async (req, res) => {
    try {
        ...
        const backendUrl = process.env.C2C_API_BASE_URL ||
        ...
        <div style="font-family: -apple-system, sans-serif; text-align: center; padding: 50px; background: #0f172a; color: white; min-height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: center;">
          <div style="background: #1e293b; padding: 36px; border-radius: 12px; border: 1px solid #ef4444; max-width: 400px; width: 100%;">
            ...
            <a href="/oauth/login?session=${encodeURIComponent(session_id)}" style="display: inline-block; padding: 10px 20px; background: #38bdf8; color: #0f172a; text-decoration: none; border-radius: 6px; font-weight: 600;">← Try Again</a>
          </div>
        </div>
      `);
        }
...
async function handleOAuthCallback(req, res) {
    try {
        const sessionId = (req.query.session || req.query.mcp_session);
        ...
        return res.status(500).json({
            error: "server_error",
            error_description: `Authentication callback failed: ${error instanceof Error ? error.message : "Unknown error"}`,
        });
    }
}
...
router.post("/token", async (req, res) => {
    try {
        ...
        if (session.clientId !== String(client_id) ||
