...
const C2C_API_BASE_URL = process.env.C2C_API_BASE_URL ||
    ...
    if (ipType === 0) {
        ...
        for (const address of addresses) {
            if ((address.family === 4 && isPrivateIPv4(address.address)) ||
                ...
                throw new Error("Image URL resolves to a private/local network address.");
            }
        }
    }
    return url;
...
export async function downloadImageForMcp(initialUrl, index) {
    ...
    for (let redirectCount = 0; redirectCount <= 3; redirectCount++) {
        ...
        try {
            ...
            throw new Error(`Failed to download image: ${error instanceof Error ? error.message : "Unknown network error"}`);
        }
        finally {
        ...
        if (response.status >= 300 && response.status < 400) {
            ...
            if (!location) {
                throw new Error("Image server returned a redirect without a Location header.");
            }
...
export async function fetchC2CBackendMultipart(endpoint, options = {}) {
    const { method = "POST", body = {}, files = [], userId, role = "buyer", requiresAuth = false, allowedRoles, } = options;
    if (!C2C_API_BASE_URL) {
        return {
            success: false,
            status: 500,
    ...
    if (requiresAuth) {
        if (!userId) {
            return {
                success: false,
                status: 401,
                error: "Authentication required: No authenticated C2C user found in MCP request context.",
            };
        }
        if (allowedRoles &&
            ...
            return {
                success: false,
                status: 403,
                error: `Permission denied: This action requires one of the following roles: [${allowedRoles.join(", ")}]. Current user role is '${role}'.`,
            };
        }
        try {
            ...
            return {
                success: false,
                status: 500,
                error: `Failed to sign C2C user token: ${error instanceof Error ? error.message : "Unknown error"}`,
            };
        }
    }
    ...
    try {
        ...
        return {
            success: false,
            status: 500,
            error: `C2C multipart API request failed: ${error instanceof Error ? error.message : "Network error"}`,
        };
    }
}
