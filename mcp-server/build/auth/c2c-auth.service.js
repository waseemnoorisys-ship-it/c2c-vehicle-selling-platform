 ...
 */
export async function generateC2CUserToken(userId, role) {
    const secret = process.env.JWT_ACCESS_SECRET;
    if (!secret) {
        throw new Error("JWT_ACCESS_SECRET is not configured in the MCP server environment.");
    }
 ...
 */
export async function verifyC2CUserToken(token) {
    const secret = process.env.JWT_ACCESS_SECRET;
    if (!secret) {
        throw new Error("JWT_ACCESS_SECRET is not configured in the MCP server environment.");
    }
    ...
    try {
        ...
        throw new Error(`C2C token verification failed: ${error instanceof Error ? error.message : "Invalid token"}`);
    }
}
/**
 ...
 */
export async function fetchC2CBackend(endpoint, options = {}) {
    const { method = "POST", body, userId, role = "buyer", requiresAuth = false, allowedRoles, } = options;
    const headers = {
