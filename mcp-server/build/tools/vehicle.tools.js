...
import { fetchC2CBackendMultipart, downloadImageForMcp, } from "../auth/c2c-multipart.service.js";
/**
...
export function registerVehicleTools(server) {
    // ============================================================
    // 1. Search Vehicles (Public Marketplace Search)
    // ============================================================
    server.registerTool("search_vehicles", {
        title: "Search Vehicles",
        description: "Search approved vehicles available on the C2C vehicle marketplace with flexible filters.",
    ...
    // ============================================================
    server.registerTool("get_vehicle_details", {
        title: "Get Vehicle Details",
        description: "Get complete specifications, images, and seller details of a vehicle listing using its listing ID.",
        inputSchema: z.object({
            listingId: z
                .string()
    ...
    // ============================================================
    server.registerTool("offer_create", {
        title: "Create Vehicle Offer",
        description: "Submit a formal purchase offer in EUR (€) for a vehicle listing on behalf of the authenticated buyer. The entire marketplace operates in EUR only.",
        inputSchema: z.object({
            listingId: z
                .string()
                .regex(/^[a-fA-F0-9]{24}$/, "listingId must be a valid 24-character hex MongoDB ObjectId")
            ...
            message: z
                ...
                .max(500)
                .optional()
                .describe("Optional note to the vehicle seller. CRITICAL: The platform is strictly EUR-only (€). Any monetary values in this message MUST strictly be written in EUR (e.g. 'I’d like to offer €43,500'). NEVER mention or write ₹ (INR), $ (USD), £ (GBP), or other non-EUR currencies."),
        }),
    ...
    // ============================================================
    server.registerTool("offer_get", {
        title: "Get Offer Details",
        description: "Get details and current status (pending, accepted, rejected) of a specific offer.",
        inputSchema: z.object({
            id: z
                .string()
                .regex(/^[a-fA-F0-9]{24}$/, "Offer id must be a valid 24-character hex MongoDB ObjectId")
    ...
    // ============================================================
    server.registerTool("offer_mine", {
        title: "Get My Offers",
        description: "Get the authenticated buyer's submitted vehicle offers from the marketplace.",
        inputSchema: z.object({
            page: z
                .number()
                .int()
    ...
    // ============================================================
    server.registerTool("offer_received", {
        title: "Get Received Offers",
        description: "Get offers received on the authenticated vendor's vehicle listings.",
        inputSchema: z.object({
            page: z
                .number()
                .int()
    ...
    // ============================================================
    server.registerTool("offer_accept", {
        title: "Accept Offer",
        description: "Accept a buyer's offer on the authenticated vendor's vehicle listing.",
        inputSchema: z.object({
            offerId: z
                .string()
                .regex(/^[a-fA-F0-9]{24}$/, "offerId must be a valid 24-character hex MongoDB ObjectId")
    ...
    // ============================================================
    server.registerTool("offer_reject", {
        title: "Reject Offer",
        description: "Reject a buyer's offer on the authenticated vendor's vehicle listing.",
        inputSchema: z.object({
            offerId: z
                .string()
                .regex(/^[a-fA-F0-9]{24}$/, "offerId must be a valid 24-character hex MongoDB ObjectId")
    ...
    // ============================================================
    server.registerTool("listing_mine", {
        title: "Get My Listings",
        description: "Get vehicle listings owned by the authenticated vendor. Optionally filter by listing status and pagination.",
        inputSchema: z.object({
            status: z
                .enum([
                "draft",
    ...
    // ============================================================
    server.registerTool("listing_create", {
    ...
    }, async ({ makeId, modelId, year, registrationNumber, mileage, fuelType, transmission, condition, askingPrice, locationText, latitude, longitude, submitForApproval, }, extra) => {
        const { userId, role } = getUserContext(extra);
    ...
    // ============================================================
    server.registerTool("listing_update", {
        title: "Update Vehicle Listing",
        description: "Update an existing vehicle listing owned by the authenticated vendor.",
        inputSchema: z.object({
            id: z
                .string()
                .regex(/^[0-9a-fA-F]{24}$/)
    ...
    }, async ({ id, makeId, modelId, year, registrationNumber, mileage, fuelType, transmission, condition, askingPrice, locationText, latitude, longitude, }, extra) => {
        const { userId, role } = getUserContext(extra);
    ...
    // ============================================================
    server.registerTool("listing_delete", {
        title: "Delete Vehicle Listing",
        description: "Delete a vehicle listing owned by the authenticated vendor.",
        inputSchema: z.object({
            id: z
                .string()
                .regex(/^[0-9a-fA-F]{24}$/)
    ...
    // ============================================================
    server.registerTool("listing_submit", {
        title: "Submit Vehicle Listing",
        description: "Submit an existing vehicle listing for approval by the authenticated vendor.",
        inputSchema: z.object({
            id: z
                .string()
                .regex(/^[0-9a-fA-F]{24}$/)
        ...
        const { userId, role } = getUserContext(extra);
        const result = await fetchC2CBackend("/api/v1/listings/submit", {
            method: "POST",
            body: {
                id,
            },
    ...
    // ============================================================
    server.registerTool("listing_photos_delete", {
        title: "Delete Listing Photo",
        description: "Delete a photo from a vehicle listing owned by the authenticated vendor.",
        inputSchema: z.object({
            id: z
                .string()
                .regex(/^[0-9a-fA-F]{24}$/)
        ...
        const { userId, role } = getUserContext(extra);
        const result = await fetchC2CBackend("/api/v1/listings/photos/delete", {
            method: "POST",
            body: {
                id,
                publicId,
    ...
    // =============================================================
    server.registerTool("listing_photos_add", {
        title: "Add Photos to Vehicle Listing",
        description: "Add one or more vehicle photos to a listing owned by the authenticated vendor. Provide publicly accessible HTTPS image URLs. Maximum 10 photos, 5 MB each. Supported formats: JPEG, PNG and WebP.",
        inputSchema: z.object({
            id: z
                .string()
                .regex(/^[0-9a-fA-F]{24}$/)
            ...
            photoUrls: z
                ...
                .min(1)
                .max(10)
                .describe("Publicly accessible image URLs. Maximum 10 photos."),
        }),
                ...
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            success: false,
                            status: 403,
                            error: "Permission denied: listing photos can only be added by vendors.",
                        }, null, 2),
                    },
                ],
                isError: true,
        ...
        try {
            ...
            for (let index = 0; index < photoUrls.length; index++) {
                const photo = await downloadImageForMcp(photoUrls[index], index);
                files.push({
                    fieldName: "photos",
            ...
            const result = await fetchC2CBackendMultipart("/api/v1/listings/photos/add", {
                method: "POST",
                body: {
                    id,
                },
    ...
    // ======================================================
    server.registerTool("listing_create_with_photos", {
        title: "Create Vehicle Listing with Photos",
        description: "Create a new vehicle listing with one or more photos for the authenticated vendor. Provide publicly accessible HTTP or HTTPS image URLs. Maximum 10 photos, 5 MB each. Supported formats: JPEG, PNG and WebP.",
        inputSchema: z.object({
            makeId: z
                .string()
                .regex(/^[0-9a-fA-F]{24}$/)
            ...
            photoUrls: z
                ...
                .min(1)
                .max(10)
                .describe("Publicly accessible image URLs. Minimum 1 and maximum 10 photos."),
        }),
    }, async ({ makeId, modelId, year, registrationNumber, mileage, fuelType, transmission, condition, askingPrice, locationText, latitude, longitude, submitForApproval, photoUrls, }, extra) => {
        const { userId, role } = getUserContext(extra);
        if (!userId) {
            return {
                content: [
                ...
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            success: false,
                            status: 403,
                            error: "Permission denied: vehicle listings can only be created by vendors.",
                        }, null, 2),
                    },
                ],
                isError: true,
        ...
        try {
            ...
            const result = await fetchC2CBackendMultipart("/api/v1/listings/create-with-photos", {
                method: "POST",
                body: {
                    makeId,
                    modelId,
    ...
    server.registerTool("notification_list", {
        ...
        description: "Get notifications for the currently authenticated C2C user. Supports pagination and filtering for unread notifications.",
        inputSchema: z.object({
            page: z
                .number()
                .int()
        ...
        const result = await fetchC2CBackend("/api/v1/notifications/mine", {
            method: "POST",
            body: {
                page: page ?? 1,
                limit: limit ?? 20,
    ...
    server.registerTool("notification_read", {
        ...
        description: "Mark one notification as read for the currently authenticated C2C user.",
        inputSchema: z.object({
            id: z
                .string()
        ...
        const result = await fetchC2CBackend("/api/v1/notifications/read", {
            method: "POST",
            body: { id },
            userId,
            role,
    ...
    server.registerTool("notification_read_all", {
        ...
        description: "Mark all unread notifications belonging to the currently authenticated C2C user as read.",
        inputSchema: z.object({}),
    }, async (_args, extra) => {
        const { userId, role } = getUserContext(extra);
        if (!userId) {
        ...
        const result = await fetchC2CBackend("/api/v1/notifications/read-all", {
            method: "POST",
            body: {},
            userId,
            role,
    ...
    // ============================================================
    server.registerTool("get_makes_and_models", {
        title: "Get Available Makes and Models",
        description: "Discover available vehicle makes and models from the C2C database. Use this to find existing makes and their valid models with MongoDB ObjectIds before creating listings or applying search filters.",
        inputSchema: z.object({
            makeName: z
                .string()
                .trim()
        ...
        const rawMakes = filterPayload.makes || [];
        const rawModelsByMake = filterPayload.models || {};
        ...
        if (makeName) {
            ...
            makesWithModels = makesWithModels.filter((m) => m.makeName.toLowerCase().includes(query) ||
    ...
    // ============================================================
    server.registerTool("create_listing_smart", {
        title: "Create Listing by Make & Model Name",
        description: "Create a vehicle listing for the authenticated vendor by providing either make & model names (e.g. 'BMW', '3 Series') or MongoDB ObjectIds. It automatically discovers and validates make/model IDs from the database.",
        inputSchema: z.object({
            make: z
                .string()
                .trim()
    ...
    }, async ({ make, model, year, mileage, fuelType, transmission, askingPrice, condition, registrationNumber, locationText, latitude, longitude, submitForApproval = true, }, extra) => {
        const { userId, role } = getUserContext(extra);
        if (!userId) {
            return {
                content: [
        ...
        const filtersResult = await fetchC2CBackend("/api/v1/landing/search-filters", {
            method: "POST",
            body: {},
            requiresAuth: false,
        });
        ...
        const makesList = filterPayload.makes || [];
        const modelsByMake = filterPayload.models || {};
        // 2. Resolve makeId
        const isMakeObjectId = /^[0-9a-fA-F]{24}$/.test(make);
        let matchedMake = isMakeObjectId
            ? makesList.find((m) => m.id === make)
            : makesList.find((m) => m.name.toLowerCase() === make.toLowerCase().trim());
        if (!matchedMake && !isMakeObjectId) {
            // Partial match fallback
            matchedMake = makesList.find((m) => m.name.toLowerCase().includes(make.toLowerCase().trim()) ||
        ...
        let matchedModel = isModelObjectId
            ...
            : availableModels.find((m) => m.name.toLowerCase() === model.toLowerCase().trim());
        if (!matchedModel && !isModelObjectId) {
            matchedModel = availableModels.find((m) => m.name.toLowerCase().includes(model.toLowerCase().trim()) ||
