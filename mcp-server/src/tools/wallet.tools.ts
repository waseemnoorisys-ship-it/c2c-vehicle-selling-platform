...
function getUserContext(extra: any): {
    ...
    const role = authInfo?.extra?.role as "buyer" | "vendor" | "admin" | undefined;
...
export function registerWalletTools(server: McpServer) {
    ...
    // ------------------------------------------------------------
    server.registerTool(
        "wallet_withdrawal_create",
        {
            title: "Request Wallet Withdrawal",
            description:
                "Submit a withdrawal request from the authenticated vendor's EUR wallet to their registered bank account. " +
            ...
            inputSchema: z.object({
                amount: z
                    ...
                    .min(100)
                    .describe(
                        "Withdrawal amount in EUR cents (integer). Minimum 100 (= €1.00). Example: 5000 = €50.00"
                    ),
            }),
        },
        async ({ amount }, extra) => {
    ...
    server.registerTool(
        "wallet_withdrawal_list",
        {
            ...
            description:
                "Retrieve a paginated list of the authenticated vendor's own withdrawal requests, ordered by most recent first. " +
    ...
    // ------------------------------------------------------------
    server.registerTool(
        "wallet_withdrawal_get",
        {
            title: "Get Withdrawal Details",
            description:
                "Retrieve full details of a specific withdrawal request by its ID. " +
            ...
            inputSchema: z.object({
                withdrawalId: z
                    ...
                    .regex(
                        /^[0-9a-fA-F]{24}$/,
                        "withdrawalId must be a valid 24-character hex MongoDB ObjectId"
                    )
    ...
    server.registerTool(
        "wallet_invoice_get",
        {
            ...
            description:
                "Retrieve or auto-generate the invoice for a completed vehicle purchase transaction. " +
