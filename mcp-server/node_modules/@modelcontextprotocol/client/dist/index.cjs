...
var OAuthClientFlowError = class extends Error {
	...
	static [Symbol.hasInstance](value) {
		return require_src.brandedHasInstance(this, value);
	}
	/**
	...
	*/
	...
	constructor(message) {
		...
		require_src.stampErrorBrands(this, new.target);
	}
};
/**
...
async function parseErrorResponse(input) {
	...
	try {
		const result = _modelcontextprotocol_core_internal.OAuthErrorResponseSchema.parse(JSON.parse(body));
...
async function discoverOAuthProtectedResourceMetadata(serverUrl, opts, fetchFn = fetch) {
	...
	return _modelcontextprotocol_core_internal.OAuthProtectedResourceMetadataSchema.parse(await response.json());
}
/**
...
async function discoverMetadataWithFallback(serverUrl, wellKnownType, fetchFn, opts) {
	...
	const protocolVersion = opts?.protocolVersion ?? _modelcontextprotocol_core_internal.LATEST_PROTOCOL_VERSION;
	let url;
...
async function discoverOAuthMetadata(issuer, { authorizationServerUrl, protocolVersion } = {}, fetchFn = fetch) {
	...
	protocolVersion ??= _modelcontextprotocol_core_internal.LATEST_PROTOCOL_VERSION;
	...
	return _modelcontextprotocol_core_internal.OAuthMetadataSchema.parse(await response.json());
}
/**
...
*/
async function discoverAuthorizationServerMetadata(authorizationServerUrl, { fetchFn = fetch, protocolVersion = _modelcontextprotocol_core_internal.LATEST_PROTOCOL_VERSION, skipIssuerValidation = false } = {}) {
	const headers = {
 ...
 /**
		...
		*/
		...
		const parsed = type === "oauth" ? _modelcontextprotocol_core_internal.OAuthMetadataSchema.parse(await response.json()) : _modelcontextprotocol_core_internal.OpenIdProviderDiscoveryMetadataSchema.parse(await response.json());
...
async function executeTokenRequest(authorizationServerUrl, { metadata, tokenRequestParams, clientInformation, addClientAuthentication, resource, fetchFn }) {
	...
	try {
		return _modelcontextprotocol_core_internal.OAuthTokensSchema.parse(json);
	} catch (parseError) {
...
async function registerClient(authorizationServerUrl, { metadata, clientMetadata, scope, fetchFn }) {
	...
	return _modelcontextprotocol_core_internal.OAuthClientInformationFullSchema.parse(await response.json());
}
...
function classifyHttpError(outcome, context) {
	if (outcome.status === 401 || outcome.status === 403) {
		...
		return {
			kind: "error",
			error: new require_src.SdkHttpError(isDenial ? require_src.SdkErrorCode.ClientHttpForbidden : require_src.SdkErrorCode.ClientHttpAuthentication, `Version negotiation failed: ${isDenial ? "the server denied access (HTTP 403)" : "the server requires authorization (HTTP 401)"}`, {
	...
	if (outcome.status >= 500) return {
		kind: "error",
		error: new require_src.SdkHttpError(require_src.SdkErrorCode.EraNegotiationFailed, `Version negotiation failed: the server answered the probe with HTTP ${outcome.status}`, {
			status: outcome.status,
			statusText: outcome.statusText,
			text: outcome.body
		})
		...
		for (;;) {
			...
			switch (verdict.kind) {
				...
				case "legacy": {
					...
					if (negotiation.kind === "pin") throw new require_src.SdkError(require_src.SdkErrorCode.EraNegotiationFailed, closedCause === void 0 ? `Version negotiation failed: the server did not offer pinned protocol version ${negotiation.version} via server/discover (no fallback in pin mode)` : `Version negotiation failed: ${closedCause} before the server offered pinned protocol version ${negotiation.version} (no fallback in pin mode)`);
					if (!negotiation.fallbackAvailable) throw new require_src.SdkError(require_src.SdkErrorCode.EraNegotiationFailed, closedCause === void 0 ? "Version negotiation failed: the server gave no modern evidence and this client supports no pre-2026-07-28 protocol version to fall back to" : `Version negotiation failed: ${closedCause} and this client supports no pre-2026-07-28 protocol version to fall back to`);
					if (closedCause !== void 0 && deps.disposableProbe !== true) throw new require_src.SdkError(require_src.SdkErrorCode.EraNegotiationFailed, `Version negotiation failed: ${closedCause} (this transport probed in place — the disposable sibling probe requires the SDK's base StdioClientTransport)`);
					return { era: "legacy" };
				}
...
function callerCloseAbortError() {
	return new require_src.SdkError(require_src.SdkErrorCode.EraNegotiationFailed, "Version negotiation failed: the transport was closed during the server/discover probe");
}
...
function serverInfoFromDiscover(discover) {
	const fromMeta = discover._meta?.[_modelcontextprotocol_core_internal.SERVER_INFO_META_KEY];
	return require_src.isSpecType.Implementation(fromMeta) ? fromMeta : void 0;
}
/**
...
function validatePrior(prior) {
	if (typeof prior === "object" && prior !== null) {
		...
		if (prior.kind === "modern" && _modelcontextprotocol_core_internal.DiscoverResultSchema.safeParse(prior.discover).success) return prior;
	}
	throw new require_src.SdkError(require_src.SdkErrorCode.EraNegotiationFailed, "connect({ prior }): unrecognized prior — expected { kind: 'modern', discover } or { kind: 'legacy' }");
}
/**
...
var Client = class extends require_src.Protocol {
	...
	*/
	async _connectFromPrior(transport, prior, options) {
		...
		const version = (explicit && require_src.modernProtocolVersions(explicit).length > 0 ? require_src.modernProtocolVersions(explicit) : require_src.SUPPORTED_MODERN_PROTOCOL_VERSIONS).find((v) => discover.supportedVersions.includes(v));
		if (version === void 0) throw new require_src.SdkError(require_src.SdkErrorCode.EraNegotiationFailed, "connect({ prior }) with a modern verdict requires a 2026-07-28+ mutual protocol version; the supplied DiscoverResult and this client's supportedProtocolVersions have no modern overlap. For a server known to be legacy, pass prior: { kind: 'legacy' } to skip the probe and initialize directly, or use versionNegotiation: { mode: 'auto' } to re-probe with legacy fallback.");
		await super.connect(transport);
	...
	*/
	async discover(options) {
		const result = await this._requestWithSchema({ method: "server/discover" }, _modelcontextprotocol_core_internal.DiscoverResultSchema, options);
	...
	*/
	_onnotification(raw, extra) {
		...
		if (raw.method === "notifications/subscriptions/acknowledged") {
			const subscriptionId = raw.params?._meta?.[_modelcontextprotocol_core_internal.SUBSCRIPTION_ID_META_KEY];
	...
	*/
	_setupListChangedHandler(listType, notificationMethod, options, fetcher) {
		const parseResult = require_src.parseSchema(_modelcontextprotocol_core_internal.ListChangedOptionsBaseSchema, options);
...
async function requestJwtAuthorizationGrant(options) {
	...
	if (!response.ok) {
		...
		const parseResult$1 = _modelcontextprotocol_core_internal.OAuthErrorResponseSchema.safeParse(errorBody);
	...
	const parseResult = _modelcontextprotocol_core_internal.IdJagTokenExchangeResponseSchema.safeParse(await response.json());
...
async function exchangeJwtAuthGrant(options) {
	...
	const parseResult = _modelcontextprotocol_core_internal.OAuthTokensSchema.safeParse(responseBody);
	...
	_startOrAuth() {
		...
		return new Promise((resolve, reject) => {
			...
			this._eventSource.onmessage = (event) => {
				...
				let message;
				try {
					message = _modelcontextprotocol_core_internal.JSONRPCMessageSchema.parse(JSON.parse(messageEvent.data));
				} catch (error) {
					this.onerror?.(error);
					return;
				}
...
var StreamableHTTPClientTransport = class {
	...
	*/
	_applyBodyDerivedHeaders(headers, message) {
		...
		const envelopeVersion = (message.params?._meta)?.[_modelcontextprotocol_core_internal.PROTOCOL_VERSION_META_KEY];
	...
	*/
	_isModernEnvelopedRequest(message) {
		...
		const v = (message.params?._meta)?.[_modelcontextprotocol_core_internal.PROTOCOL_VERSION_META_KEY];
		...
		try {
			...
			if (!response.ok) {
				...
				if (response.status === 400 && typeof text === "string" && this._isModernEnvelopedRequest(message)) try {
					const parsed = _modelcontextprotocol_core_internal.JSONRPCMessageSchema.parse(JSON.parse(text));
