...
export default function ChatInput({ conversationId }) {
  ...
  const handleSendMessage = () => {
    ...
    socketService.sendMessage(conversationId, text.trim(), "text");
    socketService.stopTyping(conversationId);
    setText("");
  };
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      if (res.success && res.data?.url) {
        socketService.sendMessage(conversationId, res.data.url, "image");
      }
    } catch (err) {
  ...
  const cancelRecording = () => {
    if (mediaRecorderRef.current && isRecording) {
      mediaRecorderRef.current.onstop = null; // Ignore onstop upload
      mediaRecorderRef.current.stop();
  ...
  const sendVoiceMessage = async (audioBlob) => {
    ...
    try {
      ...
      const res = await chatApi.uploadChatMedia(formData);
      if (res.success && res.data?.url) {
        socketService.sendMessage(conversationId, res.data.url, "audio");
      }
    } catch (err) {
  ...
  return (
    <div className="bg-[#202c33] px-4 py-3 flex items-center gap-2 border-t border-gray-700/50 relative">
